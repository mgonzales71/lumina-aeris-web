// Lumina Aeris Web Backend - Functions v1.10.0
...
export async function onRequest(context) {
    ...
    // Use environment variable for secret, or fallback to hardcoded (NOT RECOMMENDED for production)
    ...
    try {
        ...
        if (path === "/api/proxy/weather") {
            ...
            try {
                ...
                if (data.sundata) data.sundata.forEach(s => { 
                    if(s.phen === "Rise") astro.sunrise = s.time; 
                    if(s.phen === "Set") astro.sunset = s.time; 
                });
                if (data.moondata) data.moondata.forEach(m => { 
                    if(m.phen === "Rise") astro.moonrise = m.time; 
                    if(m.phen === "Set") astro.moonset = m.time; 
                });
            } catch(e) { console.error("Astro fetch failed", e); }
        ...
        if (path === "/api/context") {
            ...
            const vars = {
                ...
                "{weather}": WMO_MAP[d.current.weather_code] || "Clear",
                "{temperature}": Math.round(d.current.temperature_2m * 9/5 + 32) + "°F",
                "{theme}": "General",
                ...
                "{cloud_cover}": d.current.cloud_cover + "%",
                "{wind_speed}": d.current.wind_speed_10m + "mph",
                "{moon_phase}": astro.moon,
                "{moon_illumination}": astro.moon_illumination + " percent",
