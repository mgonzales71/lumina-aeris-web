// Lumina Aeris Web & Worker - Management Suite v1.9.7
...
<div id="data-view" class="view container">
    ...
    <div id="sub-themes"><div class="section-title">Seasonal Themes</div><div class="group" id="theme-list"></div><div style="display: flex; gap: 10px; flex-wrap: wrap;"><button onclick="addThemePrompt()" class="btn btn-secondary" style="flex:1; min-width: 140px;">➕ Add Theme</button><button onclick="importData('themes')" class="btn btn-secondary" style="flex:1; min-width: 140px;">📥 Import</button><button onclick="exportData('themes')" class="btn btn-secondary" style="flex:1; min-width: 140px;">📤 Export/View</button><button onclick="clearCategory('themes')" class="btn btn-secondary btn-danger" style="flex:1; min-width: 140px;">🗑️ Clear</button></div></div>
    ...
    <div id="sub-pois" style="display: none;">
        ...
        <div style="display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 10px;"><button onclick="discoverPOIs()" class="btn btn-secondary" style="flex:1; min-width: 140px;">✨ AI Discover</button><button onclick="addPOIPrompt()" class="btn btn-secondary" style="flex:1; min-width: 140px;">➕ Add Manual</button><button onclick="importData('pois')" class="btn btn-secondary" style="flex:1; min-width: 140px;">📥 Import Cache</button><button onclick="exportData('pois')" class="btn btn-secondary" style="flex:1; min-width: 140px;">📤 Export/View</button></div><button onclick="clearCategory('pois')" class="btn btn-secondary btn-danger">🗑️ Clear All POIs</button>
    </div>
    ...
    <div id="sub-prompts-data" style="display: none;">
        ...
        <div style="display: flex; gap: 10px; flex-wrap: wrap;">
            <button onclick="importData('prompts')" class="btn btn-secondary" style="flex:1; min-width: 140px;">📥 Import Prompts</button>
...
window.onload = async () => {
    ...
    const saved = localStorage.getItem('lumina_v1.9.7');
    if (saved) {
        try { 
        ...
        const old = localStorage.getItem('lumina_v1.9.6') || localStorage.getItem('lumina_v1.9.5') || localStorage.getItem('lumina_v1.9.4');
        if (old) { 
            try { 
...
function save() { localStorage.setItem('lumina_v1.9.7', JSON.stringify(state.settings)); }
...
function importData(type) {
    let data = prompt("Paste " + type + " JSON here:");
    if (!data) return;
    try {
        // Deep clean: remove smart quotes, invisible chars, and zero-width spaces
        data = data.trim()
            .replace(/[\\u201C\\u201D]/g, '"')
            .replace(/[\\u2018\\u2019]/g, "'")
            .replace(/[\\u200B-\\u200D\\uFEFF]/g, "");
        ...
        if (type === 'themes') { state.settings.themes = parsed; renderThemes(); }
        else if (type === 'pois') { state.settings.poiCache = parsed; renderPOISelectors(); }
        ...
        alert("Invalid JSON format. Check console."); 
        console.error(e); 
    }
}
